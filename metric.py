...
def evaluate_teacher(
    ...
    shape_names = load_shape_names(modelnet_root)
    ...
    correct = 0
    failed_count = 0
    for index in tqdm(range(points_np.shape[0]), desc="Teacher inference"):
        points = points_np[index]
        label_idx = int(labels_np[index])
        ...
        try:
            ...
            failed_count += 1
            print(f"Teacher inference failed for sample {index} ({label_name}): {exc}")
            if failed_dir.strip():
                copy_failed_images(image_paths, failed_dir, index, label_name)
        total += 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--render_dir", type=str, default="metric_render_cache", help="Teacher rendering cache/output dir")
