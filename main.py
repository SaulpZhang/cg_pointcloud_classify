...
def transform_point_cloud_to_2d(
    h5_file_list: list, labels: dict, output_dir: str, num_workers: int = 8
) -> None:
    ...
    for h5_file in h5_file_list:
        with h5py.File(h5_file, "r") as f:
            ...
            for i in range(len(f["data"])):
                points = f["data"][i]
                label_idx = (
                    int(f["label"][i][0]) if f["label"].ndim > 1 else int(f["label"][i])
                )
                label_name = labels.get(label_idx, "Unknown")

                whole_index += 1
                print(
                    f"样本 {whole_index}: 标签索引={label_idx}, 标签名称={label_name}"
                )
    ...
    if num_workers <= 1:
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            for _ in tqdm(
                executor.map(_render_one_sample, tasks),
                total=len(tasks),
                desc="Rendering point clouds",
            ):
                pass
...
if __name__ == "__main__":
    ...
    h5_file_list = [
        ...
        "scanobjectnn/main_split_nobg/training_objectdataset_augmented25rot.h5",
        "scanobjectnn/main_split_nobg/training_objectdataset_augmentedrot.h5",
        "scanobjectnn/main_split_nobg/training_objectdataset_augmentedrot_scale75.h5",
    ]
    ...
    test_h5_file_list = [
        ...
        "scanobjectnn/main_split_nobg/test_objectdataset_augmented25rot.h5",
        "scanobjectnn/main_split_nobg/test_objectdataset_augmentedrot.h5",
        "scanobjectnn/main_split_nobg/test_objectdataset_augmentedrot_scale75.h5",
    ]
